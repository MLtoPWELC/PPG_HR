...
for index_look in range(15):
    ...
    # acc_slice = numpy.load('/home/yinyibo/PycharmProjects/pytorch/PPG/DATA/PPG_FieldStudy/acc_slice_25.npy', allow_pickle=True)

    # XTrain_all = numpy.concatenate((ppg_slice.reshape((64697, 1, 200)), acc_slice.transpose((0, 2, 1))), axis=1)
    XTrain_all = ppg_slice.reshape((64697, 1, 200))
    ...
    kernel_size = 2
    dropout = 0.0011
    model = TCN(input_size=1, output_size=1, num_channels=channel_sizes,
                kernel_size=kernel_size, dropout=dropout).cuda()

    # flops, params, results = count_flops_params(model, (1, 1, 200))
